...
contract CryptoDevs is ERC721Enumerable, Ownable {
    ...
    */
    ...
    function startPresale() public onlyOwner {
        presaleStarted = true;
        // Set presaleEnded time to current timestamp + 5 minutes
        // Solidity has cool syntax for timestamps ( seconds, minutes, hours, days, years)
        presaleEnded = block.timestamp + 5 minutes;
    }

    /**
    presaleMint allows the user to mint one NFT per transaction during the presale.

    */

    function preSaleMint() public payable onlyWhenNotPaused {
        require(presaleStarted && block.timestamp < presaleEnded,  "Presale is not running now");
        require(whitelist.whitelistedAddresses(msg.sender), "Unfortunately, you are not whitelisted");
    ...
    /**
    mint allows a user to mint only one NFT per transaction after presale has ended
